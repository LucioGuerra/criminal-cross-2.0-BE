...
@ApplicationScoped
public class AssignUserToHeadquartersUseCase {
    ...
    public User execute(String firebaseUid, Long headquartersId, User currentUser) {
        ...
        if (!headquartersRepository.existsById(headquartersId)) {
            throw new DomainException("Headquarters not found");
        }
        ...
        if (!headquartersIds.add(headquartersId)) {
            throw new DomainException("User is already assigned to headquarters");
        }
